...
from tensorflow2_cifar.models import *
from tensorflow2_cifar.utils import *
...
class Model():
    ...
    def test_step(self, images, labels):
        ...
        predictions = tflib.run(predictions)
        t_loss = tflib.run(t_loss)
...
#         def train_model(train_data, train_labels, valid_data, valid_labels, input_shape, num_classes, batch_size=32, epochs=10, learning_rate=0.001):
#         model = build_model(input_shape, num_classes)
        batch_size = 128 
        
        # Compile the model
        decay_steps = int(epoch*50000./batch_size)
        
        learning_rate_fn = tf.keras.experimental.CosineDecay(initial_learning_rate=1e-1, decay_steps=decay_steps)
        
        optimizer = tf.keras.optimizers.SGD(learning_rate=learning_rate_fn, momentum=0.9)
        self.model.compile(optimizer=optimizer, loss='categorical_crossentropy', metrics=['accuracy'])

        # Train the model
        history = self.model.fit(train_ds, validation_data=test_ds, epochs=epoch)
        
        self.model.save_weights('./resnet18_cifar10_255')
...
#                     sess.run(self.train_step(images, labels))

#                 for images, labels in test_ds:
#                     images = tf.dtypes.cast(images, tf.float32)
# #                     self.test_step(images, labels)
#                     sess.run(self.test_step(images, labels))
...
def main():
    ...
    test_ds.map(lambda x, y: (tf.cast(x, tf.float32), y))
