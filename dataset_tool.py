...
def create_cifar10(tfrecord_dir, cifar10_dir, ignore_labels):
    ...
    sizes = [5000, 3871, 2997, 2320, 1796, 1391, 1077, 834, 645, 500]
...
#     np.random.shuffle(images)
    ...
    with TFRecordExporter(tfrecord_dir, images.shape[0]) as tfr:
        for idx in range(len(images)):
            print(dict_counts)
            if labels[idx] not in dict_counts.keys():
                dict_counts[labels[idx]] = 0
            

            if dict_counts[labels[idx]] > sizes[labels[idx]]:
                continue
            else:
                dict_counts[labels[idx]] += 1
