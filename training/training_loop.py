...
from training import dataset_combined as dataset
...
def training_loop(
    ...
    with tf.device('/gpu:0'):
        ...
        D = tflib.Network('D', num_channelsi=training_set.shape[0], resolution=training_set.shape[1], label_size=training_set.label_size, **D_args)
        Gs = G.clone('Gs')
        race_model = ResNet('resnet18', 10)
        race_model_path = "/scratch/aj3281/stylegan2-ada/resnet18_cifar10_255" #"/home/aj3281/scratch/stylegan2-ada/tensorflow2-cifar/checkpoints/resnet18/"
...
#         race_model = ResNet('resnet18', 10)
#         race_model.load_weights("/scratch/aj3281/stylegan2-ada/resnet18_cifar10_255")
...
#         race_model = Race.loadModel()
    ...
    for gpu, (G_gpu, D_gpu) in enumerate(zip(G_gpus, D_gpus)):
        with tf.name_scope(f'Train_gpu{gpu}'), tf.device(f'/gpu:{gpu}'):
            ...
            bias_lr = 0
            if bias_loss is True:
                bias_lr = 1

            print("bias learning rate", bias_lr)

            terms = dnnlib.util.call_func_by_name(G=G_gpu, D=D_gpu, aug=aug, race_model=race_model, bias_lr=bias_lr, fake_labels=fake_labels, real_images=real_images_var, real_labels=real_labels_var, **loss_args)
            if lazy_regularization:
