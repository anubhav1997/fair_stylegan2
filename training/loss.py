...
def stylegan2(G, D, aug, race_model, bias_lr, fake_labels, real_images, real_labels, r1_gamma=10, pl_minibatch_shrink=2, pl_decay=0.01, pl_weight=2, **_kwargs):
    ...
    with tf.name_scope('Loss_main'):
        ...
        loss_type ='bias' #'reweighting'

        if loss_type == 'reweighting':
#             lambda_race = [0.9534, 64.10, 9.74, 0.2549, 5.4466, 1.3740]
#             lambda_race = [0.16504, 0.19948, 0.19658, 0.06928, 0.19388, 0.17574]
#             lambda_race = [0.6446, 0.6532, 0.7482, 0.8295, 0.9418, 1.0672, 1.2163, 1.2762, 1.4269, 14.1507]
            lambda_race = [0.40862, 0.52779643503, 0.68171504838, 0.88064655172, 1.13758351893, 1.46879942487, 1.89702878366, 2.44976019185, 3.16759689922, 4.0862] #long tail weights 
            fake_images_out = tf.transpose(G_fake.images, perm=[0, 2, 3, 1])
#             imgs = tf.image.resize(fake_images_out, (224, 224))
            imgs = fake_images_out 
            preds = race_model(imgs)
            preds = tf.math.argmax(preds, 1)
            preds = tf.cast(preds, tf.int32)
            # weights = tf.convert_to_tensor(lambda_race)[preds]
            weights = tf.gather(tf.convert_to_tensor(lambda_race), preds)
            G_loss = tf.tensordot(weights, tf.nn.relu(G_loss), axes = 1)


        elif bias_lr > 0.0:
            #lambda_race = [0.1 for i in range(10)]
#             lambda_race = [0.9534, 64.10, 9.74, 0.2549, 5.4466, 1.3740]
            # lambda_bias = 0.5
            #lambda_race = [0.6446, 0.6532, 0.7482, 0.8295, 0.9418, 1.0672, 1.2163, 1.2762, 1.4269, 14.1507]
            lambda_race = [0.40862, 0.52779643503, 0.68171504838, 0.88064655172, 1.13758351893, 1.46879942487, 1.89702878366, 2.44976019185, 3.16759689922, 4.0862] #long tail weights 
            ...
            #lambda_race = [1./6, 1./6, 1./6, 1./6, 1./6, 1./6]
...
#                imgs = imgs/255.
            ...
            preds = tf.one_hot(preds, depth=len(lambda_race))

            preds = tf.reduce_sum(preds, axis=0)

            E_Xr = (1.0 / tf.cast(minibatch_size, tf.float32)) * preds #imgs.shape[0].value * preds

            loss_bias = tf.tensordot(tf.convert_to_tensor(lambda_race), tf.nn.relu(1./len(lambda_race) - E_Xr), axes = 1)
            G_loss += bias_lr * loss_bias 
