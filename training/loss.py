...
def stylegan2(G, D, aug, race_model, bias_lr, fake_labels, real_images, real_labels, r1_gamma=10, pl_minibatch_shrink=2, pl_decay=0.01, pl_weight=2, **_kwargs):
    ...
    with tf.name_scope('Loss_main'):
        ...
        if bias_lr > 0.0:
            ...
            # preds = tf.math.argmax(preds, 0)
            preds = tf.math.argmax(preds, 1)

            preds = tf.one_hot(preds, depth=len(lambda_race))  # preds.shape[0])

            # preds = tf.one_hot(preds, depth=minibatch_size)#preds.shape[0])

            preds = tf.reduce_sum(preds, axis=0)

            E_Xr = (1.0 / tf.cast(minibatch_size, tf.float32)) * preds #imgs.shape[0].value * preds

            loss_bias = tf.tensordot(tf.convert_to_tensor(lambda_race), tf.nn.relu(E_Xr), axes = 1)
            # loss += lambda_bias * loss_bias #bias_loss(fake_images_out)
