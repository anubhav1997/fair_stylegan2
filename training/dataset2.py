...
class TFRecordDataset:
    def __init__(self,
        ...
        self.tfrecord_dir2      = tfrecord_dir2
        ...
        self.has_validation_set = (self._max_validation > 0) and any(
            os.path.basename(f).startswith('validation-') for f in all_files2)
        ...
        if self.label_file2 is None:
            guess = [f for f in all_files2 if f.endswith('.labels')]
            if len(guess):
                self.label_file2 = guess[0]
        elif not os.path.isfile(self.label_file2):
            guess = os.path.join(self.tfrecord_dir2, self.label_file2)
            if os.path.isfile(guess):
                self.label_file2 = guess
        ...
        # self.resolution = resolution if resolution is not None else max_shape2[1]
        # self.resolution_log2 = int(np.log2(self.resolution))
        # self.shape = [max_shape[0], self.resolution, self.resolution]
        # tfr_lods2 = [self.resolution_log2 - int(np.log2(shape[1])) for shape in tfr_shapes]
        # assert all(shape[0] == max_shape[0] for shape in tfr_shapes)
        # assert all(shape[1] == shape[2] for shape in tfr_shapes)
        # assert all(shape[1] == self.resolution // (2**lod) for shape, lod in zip(tfr_shapes, tfr_lods))
        # assert all(lod in tfr_lods for lod in range(self.resolution_log2 - 1))
        ...
        with tf.name_scope('Dataset'), tf.device('/cpu:0'), tf.control_dependencies(None):
            ...
            for tfr_file, tfr_shape, tfr_lod, tfr_file2, tfr_shape2 in zip(tfr_files, tfr_shapes, tfr_lods, tfr_files2, tfr_shapes2):
                if tfr_lod < 0:
                    continue
