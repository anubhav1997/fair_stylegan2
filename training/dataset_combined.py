...
class TFRecordDataset:
    def __init__(self,
        ...
        self.tfrecord_dir2       = '/scratch/aj3281/data/syn_cifar_long_tail_new/'
        self.resolution2         = None
        self.resolution_log22    = None
        self.shape2              = []        # [channels, height, width]
        self.dtype2              = 'uint8'
        self.label_file2         = label_file
        self.label_size2         = None      # components
        self.label_dtype2        = None
        self.has_validation_set2 = None
        self.mirror_augment2     = mirror_augment
        self.repeat2             = repeat
        self.shuffle2            = shuffle
        self._max_validation2    = max_validation
        self._np_labels2         = None
        self._tf_minibatch_in2   = None
        self._tf_labels_var2     = None
        self._tf_labels_dataset2 = None
        self._tf_datasets2       = dict()
        self._tf_iterator2       = None
        self._tf_init_ops2       = dict()
        self._tf_minibatch_np2   = None
        self._cur_minibatch2     = -1
        self._cur_lod2           = -1
        
        # List files in the dataset directory.
        assert os.path.isdir(self.tfrecord_dir2)
        all_files2 = sorted(glob.glob(os.path.join(self.tfrecord_dir2, '*')))
        self.has_validation_set2 = (self._max_validation2 > 0) and any(os.path.basename(f).startswith('validation-') for f in all_files2)
        all_files2 = [f for f in all_files2 if os.path.basename(f).startswith('validation-') == _is_validation]

        # Inspect tfrecords files.
        tfr_files2 = [f for f in all_files2 if f.endswith('.tfrecords')]
        assert len(tfr_files2) >= 1
        tfr_shapes2 = []
        for tfr_file2 in tfr_files2:
            tfr_opt2 = tf.python_io.TFRecordOptions(tf.python_io.TFRecordCompressionType.NONE)
            for record in tf.python_io.tf_record_iterator(tfr_file2, tfr_opt2):
                tfr_shapes2.append(self.parse_tfrecord_np(record).shape)
                break

        # Autodetect label filename.
        if self.label_file2 is None:
            guess2 = [f for f in all_files2 if f.endswith('.labels')]
            if len(guess2):
                self.label_file2 = guess2[0]
        elif not os.path.isfile(self.label_file2):
            guess2 = os.path.join(self.tfrecord_dir2, self.label_file2)
            if os.path.isfile(guess2):
                self.label_file2 = guess2

        # Determine shape and resolution.
        max_shape2 = max(tfr_shapes2, key=np.prod)
        self.resolution2 = resolution if resolution is not None else max_shape2[1]
        self.resolution_log22 = int(np.log2(self.resolution2))
        self.shape2 = [max_shape2[0], self.resolution2, self.resolution2]
        tfr_lods2 = [self.resolution_log22 - int(np.log2(shape[1])) for shape in tfr_shapes2]
        assert all(shape[0] == max_shape[0] for shape in tfr_shapes2)
        assert all(shape[1] == shape[2] for shape in tfr_shapes2)
        assert all(shape[1] == self.resolution2 // (2**lod) for shape, lod in zip(tfr_shapes2, tfr_lods2))
        assert all(lod in tfr_lods2 for lod in range(self.resolution_log22 - 1))
        
        max_label_size2 = max_label_size
        max_images2 = max_images
        # Load labels.
        assert max_label_size2 == 'full' or max_label_size2 >= 0
        self._np_labels2 = np.zeros([1<<30, 0], dtype=np.float32)
        if self.label_file2 is not None and max_label_size2 != 0:
            self._np_labels2 = np.load(self.label_file2)
            assert self._np_labels2.ndim == 2
        if max_label_size2 != 'full' and self._np_labels2.shape[1] > max_label_size2:
            self._np_labels2 = self._np_labels2[:, :max_label_size2]
        if max_images2 is not None and self._np_labels2.shape[0] > max_images2:
            self._np_labels2 = self._np_labels2[:max_images2]
        self.label_size2 = self._np_labels2.shape[1]
        self.label_dtype2 = self._np_labels2.dtype.name
        ...
        with tf.name_scope('Dataset'), tf.device('/cpu:0'), tf.control_dependencies(None):
            ...
            for tfr_file, tfr_shape, tfr_lod, tfr_file2, tfr_shape2, tfr_lod2 in zip(tfr_files, tfr_shapes, tfr_lods, tfr_files2, tfr_shapes2, tfr_lods2):
                if tfr_lod < 0:
                    continue
                dset = tf.data.TFRecordDataset(tfr_file, compression_type='', buffer_size=buffer_mb<<20)
                dset = dset.apply(tf.data.experimental.ignore_errors())
                
                dset2 = tf.data.TFRecordDataset(tfr_file2, compression_type='', buffer_size=buffer_mb << 20)
                dset2 = dset2.apply(tf.data.experimental.ignore_errors())

                #dset = dset.concatenate(dset2)
                #dset = dset.shuffle(5000)
                dset = tf.data.experimental.sample_from_datasets([dset, dset2], weights=[0.5, 0.5])
